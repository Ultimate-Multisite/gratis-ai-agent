 ...
 *  2. The `bin/build.sh --target=wporg` distribution can physically strip this
 *     file (see `.distignore-wporg`) AND force
 *     `SD_AI_AGENT_FEATURE_USER_MANAGEMENT` to `false` in the bundled main
 *     file, providing defence-in-depth removal of user create/login surface
 *     in line with the WordPress.org plugin review feedback that custom user
 *     creation routes can bypass security plugins (e.g. login throttling,
 *     password-policy enforcers) that hook the native register/login flow.
 *
...
class UserManagementAbilities {
	 ...
	 *
	 * Gated upstream by `Features::is_enabled( Features::USER_MANAGEMENT )`
	 * and a `class_exists()` guard in `AbilitiesHandler` so the wp.org
	 * distribution (where this class is stripped from the zip and the
	 * backing constant is forced to `false`) becomes a no-op.
	 */
