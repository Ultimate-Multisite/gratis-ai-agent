 ...
 *
 * Provides the fluent prompt builder for the AI Client bridge
 * on WordPress < 7.0 where this class is not available in core.
 *
 * On WordPress 7.0+, this file is a no-op — core's definition wins.
 *
 * Source: wp-includes/ai-client/class-wp-ai-client-prompt-builder.php (WP 7.0)
 *
	...
	public function __call( string $name, array $arguments ) {
		...
		if ( self::is_support_check_method( $name ) || self::is_generating_method( $name ) ) {
			...
			if ( $prevent ) {
				...
				? __( 'AI features are not supported in this environment.', 'superdav-ai-agent' )
				: __( 'Prompt execution was prevented by a filter.', 'superdav-ai-agent' );
