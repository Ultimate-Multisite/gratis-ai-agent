...
class Agent {
	...
	private static function build_setup_assistant_prompt( string $site_title, string $site_url ): string {
		...
		$phase_2 = "## Phase 2: Build or explicitly rebuild the homepage\n\n"
			...
			. "5. Curate the homepage imagery before composing. Reuse user uploads first. Otherwise call `sd-ai-agent/stock-image` with `action: search`, `usage: hero`, landscape orientation, and at least 12 candidates; consider only candidates returned above the role-specific quality floor, select for subject relevance and a coherent visual set, then import the exact reviewed candidate. A filename or search rank is not visual evidence. Never use a watermarked, signed, preview-sized, visibly compressed, or stylistically inconsistent image. Treat `success: false` or an `error` as a failed search or import. After `action: import`, also treat `attachment_id: 0` or an empty `url` as a failed acquisition, never as completed imagery. Use the returned local `attachment_id` or local media URL only after a successful import, never an external thumbnail. If stock acquisition fails or no candidate passes, immediately use `sd-ai-agent/generate-image` for a brand-specific composition when available; otherwise report the specific failure and do not publish a homepage that requires primary media as weak/text-only.\n"
