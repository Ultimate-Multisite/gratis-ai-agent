...
class Agent {
	...
	private static function build_setup_assistant_prompt( string $site_title, string $site_url ): string {
		...
		$phase_2 = "## Phase 2: Build the homepage (empty-install branch only)\n\n"
			...
			. "5. Build one complete schema-v1 design-token contract for the chosen direction and call `sd-ai-agent/compile-design-tokens`. If it returns an error (including contrast failures), repair the contract and retry compilation before continuing. Do not independently reconstruct theme.json or Global Styles payloads.\n"
			. "6. Call `sd-ai-agent/validate-palette-contrast` on the compiler's returned `palette`. If it reports missing roles or failures, repair the contract, recompile it, and call `sd-ai-agent/validate-palette-contrast` again. Call `sd-ai-agent/scaffold-block-theme` only after a validation result whose `passed` field is exactly `true`; a failed, missing, or unevaluated result must never proceed to scaffolding.\n"
			. "7. Call `sd-ai-agent/scaffold-block-theme` with the inferred metadata and the compiler's `theme_json` output. It must use schema **version 3** (never v2) with `\"\$schema\": \"https://schemas.wp.org/trunk/theme.json\"` and `\"version\": 3`.\n"
			. "8. Write `parts/header.html`, `parts/footer.html`, `templates/index.html`, `templates/page.html`, and `templates/front-page.html` via `sd-ai-agent/file-write`. Validate each one with `sd-ai-agent/validate-block-content`.\n"
			. "9. Persist the compiler's non-empty `global_styles.settings` and `global_styles.styles` through `sd-ai-agent/update-global-styles`. Never reconstruct raw color, typography, spacing, or element-style payloads independently and never pass empty arrays/objects.\n"
			. "10. Publish the homepage as a real page via `sd-ai-agent/create-post` (post_type: page, status: publish). Compose hero + about + primary CTA using:\n"
