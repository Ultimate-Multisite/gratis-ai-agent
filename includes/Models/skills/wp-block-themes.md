...
8. After activation, run `sd-ai-agent-js/validate-theme-completion` with the active generated stylesheet, the current `validate-block-theme-project` fingerprint, `homepage_url` set to the real homepage, and `interior_url` set to one published interior-page URL. It must finish all six page/viewport renders at **375×812**, **768×1024**, and **1280×800**. This is a hard completion gate, not advisory review: browser unavailability, timeout, partial execution, standalone preview HTML, cached screenshots, subjective approval, or structural-only review never passes. When browser execution is unavailable, record one terminal blocker and never replay it as synthetic conversation turns; resume completion only from a browser-capable client.
9. Repair every report violation (active stylesheet, render health, overflow, landmarks/headings/language/duplicate IDs, image alt treatment, accessible names/focus, contrast, placeholders/CTA/remote images/broken assets, local-asset response status, booking-widget availability, actionable checkout paths, complete contact controls, and reduced-motion visibility). Any theme, page, Global Styles, or style-variation mutation invalidates the report: rerun project validation, activate the current stylesheet where needed, and rerun the browser validator. Write every referenced local asset before activation. If every frontend render is unrenderable after activation, restore the previous stylesheet rather than declaring completion.
...
Compilation is pure. Call `sd-ai-agent/compile-design-tokens` and follow its complete schema-v1 contract exactly (governance identity, bounded primitives, semantic roles, and a style-variation remap); repair any `WP_Error` and never reconstruct raw artifacts independently. For an invalid primitive response, repair only the reported `path` using its `expected` CSS value shape, then retry compilation; never substitute raw CSS declarations, selectors, URLs, or an unbounded CSS expression. Pass `theme_json` to `sd-ai-agent/scaffold-block-theme`, pass `global_styles.settings` and `global_styles.styles` only to `sd-ai-agent/update-global-styles`, validate `palette` with `sd-ai-agent/validate-palette-contrast`, and preserve the file-only `style_variation` plus `artifact_manifest` for `sd-ai-agent/apply-design-artifact-release`; the manifest must never own a second `wp_global_styles` record.
...
When selected, follow its measurable `hero_contract` as well as ordered section roles, the `core/*` block allowlist, responsive behavior, and accessibility requirements. The contract chooses immersive, split, product, or editorial composition from visitor goal and available content rather than hard-coding vertical-specific layouts. An `immersive-media` hero requires an `alignfull` Cover or equivalent full-bleed media wrapper and a source large enough for its rendered width. Keep visual direction and design-token selection separate.
...
7. For generated themes or site scaffolds, completion requires both a passing `sd-ai-agent-js/validate-theme-completion` report for the active stylesheet/current project fingerprint and the Setup Assistant's current `sd-ai-agent-js/validate-page-quality` report for affected published page mutations. Validate the real homepage plus one published interior page at 375×812, 768×1024, and 1280×800; retain URL, viewport, selector/evidence, severity, remediation, media-resolution, template-ownership, and first-impression data. Do not replace either report with preview HTML, screenshots, a structural checklist, or an unavailable-browser disclaimer.
## See also
