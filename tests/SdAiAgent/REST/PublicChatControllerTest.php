...
class PublicChatControllerTest extends WP_UnitTestCase {
	...
	public function test_public_chat_routes_are_registered(): void {
		...
		$this->assertArrayHasKey( '/sd-ai-agent/v1/public-chat/session', $routes );
	...
	public function test_allowed_origin_creates_public_job_without_user_or_cookies(): void {
		...
		$this->assertArrayNotHasKey( 'job_token', $data );
	...
	public function test_public_job_polling_requires_public_token(): void {
		...
		$session_response = $this->dispatch( 'POST', '/sd-ai-agent/v1/public-chat/session' );
		$session_token    = $session_response->get_data()['token'];
		$session_uuid     = $this->extract_session_uuid_from_token( $session_token );
		...
		set_transient(
			...
			array(
				...
				'public_token_hash'   => hash( 'sha256', $session_token ),
		...
		$response = $this->dispatch( 'GET', "/sd-ai-agent/v1/public-chat/job/{$job_id}", array( 'token' => $session_token ) );
		$this->assertSame( 200, $response->get_status() );
	...
	/**
	 * Extract the signed session UUID from a test token.
	 *
	 * @param string $token Public chat session token.
	 * @return string
	 */
	private function extract_session_uuid_from_token( string $token ): string {
		$parts = explode( '.', $token, 2 );
		$this->assertCount( 2, $parts );

		// phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- Test decodes signed public token payload to seed a matching job fixture.
